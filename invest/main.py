...
from invest.db.engine import init_db, get_session
from invest.db.models import AssetType
from invest.data.fetcher import fetch_all_prices, fetch_market_pulse, fetch_ohlcv_for_symbol
...
def _ensure_db() -> None:
    try:
        ...
        console.print(f"[bold red]Database init failed:[/bold red] {e}")
        sys.exit(1)
...
def add(ticker: str, quantity: float, price: float):
    """Add a new position.
    ...
    try:
        ...
        console.print(f"[bold red]Failed to add position:[/bold red] {e}")
        sys.exit(1)
...
@click.argument("ticker")
@click.argument("price", type=float)
def close(ticker: str, price: float):
    ...
    async def _run():
        async with get_session() as session:
            ...
            if not matches:
                console.print(f"[yellow]No active position found for {ticker.upper()}.[/yellow]")
                return

            if len(matches) == 1:
                target = matches[0]
            else:
                table = Table(box=box.SIMPLE, title=f"Multiple positions for {ticker.upper()}")
    ...
    try:
        ...
        console.print(f"[bold red]Failed to close position:[/bold red] {e}")
        sys.exit(1)
...
def detect_cmd():
    ...
    async def _run():
        ...
        alerts.sort(
            key=lambda a: (0 if a.level.value == "CRITICAL" else 1, -abs(a.detail.get("z_score", 0))),
        )
        ...
        console.print(Panel(table, title="Anomaly Detection Results", border_style="yellow"))
